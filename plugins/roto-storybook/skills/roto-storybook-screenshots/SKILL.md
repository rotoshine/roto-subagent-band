...
- **Supabase Storage (선택)**: 아래 환경변수가 `.env` 또는 `.env.local`에 설정되어 있으면 git 대신 Supabase Storage에 업로드합니다. 없으면 기존 git 방식으로 동작합니다.
  - `SUPABASE_URL` — Supabase 프로젝트 URL
  - `SUPABASE_ANON_KEY` — Supabase anon key (public bucket + RLS INSERT 정책 필요)
  - `SUPABASE_STORAGE_BUCKET` — 업로드할 bucket 이름
...
### Step 6: 이미지 저장 (Supabase Storage 또는 Git)

환경변수를 확인하여 저장 방식을 자동 결정합니다.

```
환경변수 탐색 순서:
1. 현재 작업 디렉토리의 .env, .env.local
2. 모노레포인 경우 각 서비스 디렉토리의 .env, .env.local (예: web-services/web/.env)
3. 위 파일들에서 SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_STORAGE_BUCKET 확인
```

#### 방식 A: Supabase Storage (환경변수 있을 때)

**조건**: `SUPABASE_URL` + `SUPABASE_ANON_KEY` + `SUPABASE_STORAGE_BUCKET` 세 값이 모두 존재

각 스크린샷을 Supabase Storage에 업로드합니다:

```bash
# 업로드 경로: {branch}/{story-name}-{light|dark}.png
# upsert: true — 같은 이름이면 덮어씀

curl -X POST \
  "${SUPABASE_URL}/storage/v1/object/${SUPABASE_STORAGE_BUCKET}/${BRANCH}/${filename}" \
...
**이미지 URL 형식**: `${SUPABASE_URL}/storage/v1/object/public/${SUPABASE_STORAGE_BUCKET}/${BRANCH}/${filename}`

git에는 스크린샷을 추가하지 않습니다.

#### 방식 B: Git 스테이징 (환경변수 없을 때)
...
| 상태 | Light | Dark |
|------|-------|------|
| 기본 | ![light]({image-url}-light.png) | ![dark]({image-url}-dark.png) |
| 입장완료 | ![light]({image-url}-light.png) | ![dark]({image-url}-dark.png) |
```

이미지 URL은 Step 6에서 결정된 방식(Supabase public URL 또는 GitHub raw URL)을 사용합니다.

PR이 없으면:
- Supabase 방식: 업로드된 URL 목록만 출력
- Git 방식: 스크린샷 파일만 stage하고 알림
